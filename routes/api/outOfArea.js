...
router.post('/', async(req, res) => {
            ...
            router.post('/', async(req, res) => {
                ...
                for (let i = 0; i < req.body.path.length; i++)
                    if (!req.body.path[i].latitude || !req.body.path[i].longitude || !req.body.path[i].date) return res.status(400).json({ status: 400, statusCode: 'failed', message: 'Each object must contain latitude, longitude and date fields' })
